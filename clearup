...
if ! command -v crontab &> /dev/null; then
    echo "错误：crontab 未安装，请手动安装后再运行脚本"
    exit 1
fi
...
#description: 清理 /tmp 目录中过期的临时文件
echo "清理 /tmp 目录中过期的临时文件..."
...
# 配置定时任务
echo "配置定时任务（每天凌晨4点运行）..."
(crontab -l 2>/dev/null; echo "0 4 * * * sh /opt/script/cron/cleanCache.sh") | crontab - || { echo "错误：定时任务配置失败"; exit 1; }
...
echo "====================================="
echo "脚本执行完毕！"
echo "定时任务已设置，每天凌晨4点运行 /opt/script/cron/cleanCache.sh"
echo "====================================="
